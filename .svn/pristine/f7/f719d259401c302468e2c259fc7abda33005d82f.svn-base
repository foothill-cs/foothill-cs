...
function Navbar() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const title = document.getElementById('foothillCS');
  ...
  return (
    <section id="navbar" className={`${styles.navbar}`}>
      <div>
        <Link href="/">
          <a className="flex items-center">
            ...
            <h2 id="foothillCS" className={styles.navTitle}>
              Foothill CS Club
            </h2>
          </a>
        </Link>
      ...
      <div id="navList" className={styles.navbarList}>
        ...
        <a
          href="https://www.google.com/search?q=we+need+a+sign+up+google+form&oq=we+need+a+sign+up+google+form&gs_lcrp=EgZjaHJvbWUyBggAEEUYOTIGCAEQRRg70gEINTIwMWowajGoAgCwAgA&sourceid=chrome&ie=UTF-8"
          target="_blank"
